...
def save_gpt_input_img(images, save_pth, view_ind=0):
    # Create plot
...
def gpt_phys_prismatic_annotation_main(part_seg_dir, save_res_dir, entity, gpt_config):
    gpt_save_dir = os.path.join(save_res_dir, "gpt_prismatic_annotation")
    ...
    annotation_axis_dir = os.path.join(save_res_dir, "obj_prismatic")
    ...
    for part in parts:
        ...
        img_save_dir = os.path.join(part_dir, "gpt")
        os.makedirs(img_save_dir, exist_ok=True)

        image_path_list = []
        shutil.copy(os.path.join(part_seg_dir, entity, "gpt", f"{part}.png"),
                    os.path.join(img_save_dir, "1.png"))
        image_path_list.append(os.path.join(img_save_dir, "1.png"))
        for i in range(2):
            img_pths = [
                os.path.join(part_dir, f"imgs/{i}_false.png"),
                os.path.join(part_dir, f"imgs/{i}_true.png")
            ]
            save_gpt_input_img(img_pths, os.path.join(img_save_dir, f"{i + 2}.png"), view_ind=i)
            image_path_list.append(os.path.join(img_save_dir, f"{i + 2}.png"))

        save_anno_file = os.path.join(gpt_save_dir, entity, f'{part}.json')
        os.makedirs(os.path.dirname(save_anno_file), exist_ok=True)
        if not os.path.exists(save_anno_file):
            ...
            axis_npy = np.load(os.path.join(annotation_axis_dir, entity, part, "axis.npy"))
            axis_npy = np.round(axis_npy, 5)
            prompt = f"""You are a motion-axis prediction model.
Given the following inputs, determine which single candidate axis best represents the true motion axis of the target 3D part.
...
* Motion type: PRISMATIC (linear sliding motion).
...
**Select exactly one axis**, output following information in JSON:
