...
def hierarchical_clustering_labels(children, n_samples, cluster_num_tuple):
    start_cluster_num = cluster_num_tuple[0]
    end_cluster_num = cluster_num_tuple[1]
    ...
    for i, (child1, child2) in enumerate(children):
        ...
        if end_cluster_num >= current_cluster_count >= start_cluster_num:
...
def solve_clustering(input_fname, uid, view_id, save_dir="test_results1", out_render_fol="test_render_clustering",
                     use_agglo=False, init_num_clusters=10, search_scope=(3, 20), is_pc=False,
                     option=1, with_knn=True, export_mesh=True):
    print(uid, view_id)

    start_cluster_num = init_num_clusters - search_scope[0]
    end_cluster_num = init_num_clusters + search_scope[1]
    if start_cluster_num < 1:
        start_cluster_num = 1

    print("start_cluster_num:", start_cluster_num)
    print("end_cluster_num:", end_cluster_num)
    ...
    hierarchical_labels = hierarchical_clustering_labels(clustering.children_, point_feat.shape[0],
                                                         cluster_num_tuple=(start_cluster_num, end_cluster_num))

    all_FL = []

    for n_cluster in range(end_cluster_num - start_cluster_num + 1):
    ...
    for n_cluster in range(end_cluster_num - start_cluster_num + 1):
        ...
        if export_mesh:
            fname_mesh = os.path.join(out_render_fol, "ply",
                                      str(uid) + "_" + str(view_id) + "_" + str(end_cluster_num - n_cluster).zfill(
                                          2) + ".ply")
            export_colored_mesh_ply(V, F, FL, filename=fname_mesh)

        fname_clustering = os.path.join(out_render_fol, "cluster_out",
                                        str(uid) + "_" + str(view_id) + "_" + str(end_cluster_num - n_cluster).zfill(
                                            2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init_num_clusters', default=10, type=int)
    parser.add_argument('--search_scope', nargs="+", default=[3, 20], type=int)
    ...
    parser.add_argument('--export_mesh', default=False, type=bool)

    # --root ./exp_results/partfield_features/abo
    # --dump_dir ./exp_results/clustering/abo
    # --source_dir ./data/abo
    # --use_agglo True
    # --max_num_clusters 20
    # --option 0
    ...
    INIT_NUM_CLUSTERS = FLAGS.init_num_clusters
    SEARCH_SCOPE = FLAGS.search_scope
    ...
    all_files = []
    if os.path.exists(os.path.join(OUTPUT_FOL, "ply")):
    ...
    for model in selected:
        ...
        solve_clustering(fname, uid, view_id,
                         save_dir=root,
