...
png_path = sys.argv[9]
save_center_dir = sys.argv[10]
view_index = int(sys.argv[11])
...
#record_pth = "./record.json"
record_pth = os.path.join(save_center_dir, "center.json")
import json
if not os.path.exists(record_pth):
    record = {"center": list(center), "dist": dist}
    json.dump(record, open(record_pth, "w"), indent=4)
else:
    record = json.load(open(record_pth, "r"))
    center = mathutils.Vector(record["center"])
    dist = record["dist"]
    print("li", center, dist)
# -------------------------
# 渲染循环
# -------------------------
