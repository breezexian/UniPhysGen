...
def render_face(glb_path, output_dir, batch_idx, start, end):
    # -------------------------
    # 参数设置
    # -------------------------
    ...
    #     return
    target_face_indices = set(range(start, end, 1))

    # 默认背景材质
    default_mat = bpy.data.materials.new(name=f"Mat_0")
    default_mat.pass_index = 0
    mesh.materials.append(default_mat)

    for i, poly in enumerate(mesh.polygons):
        # 创建一个新材质
        if i in target_face_indices:
            mat = bpy.data.materials.new(name=f"Mat_{i}")
            mat.pass_index = i - start + 1  # 每个面唯一 ID
            mesh.materials.append(mat)
            poly.material_index = len(mesh.materials) - 1  # 绑定到面
            #print(i-start+1, len(mesh.materials)-1)
        else:
            poly.material_index = 0  # 绑定到面

        vert_key = list(map(str, list(poly.vertices)))  # 顶点索引list
        # mat_id = mesh.materials[f"Mat_{poly.material_index}"].pass_index
        face_id_dict["-".join(vert_key)] = i + 1  # mat.pass_index 作为面ID

    json_output_pth = os.path.join(output_dir, f"face2id.json")
    ...
    for name, pos in cam_positions.items():
        ...
        # 设置输出路径
        file_output_index.file_slots[0].path = f"{name}_{batch_idx}_faceID"
...
if __name__ == '__main__':
    try:
        ...
        save_dir = sys.argv[9]
        num_faces = int(sys.argv[10])
        MAX_FACES_PER_BATCH = int(sys.argv[11])
        num_batches = math.ceil(num_faces / MAX_FACES_PER_BATCH)
        print("======")
        for batch_idx in range(num_batches):
            start = batch_idx * MAX_FACES_PER_BATCH
            end = min((batch_idx + 1) * MAX_FACES_PER_BATCH, num_faces)
            print(f"Processing batch {batch_idx + 1}/{num_batches}, faces {start} to {end}")
            render_face(model_path, save_dir, batch_idx, start, end)
