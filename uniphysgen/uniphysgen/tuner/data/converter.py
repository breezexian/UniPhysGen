...
if TYPE_CHECKING:
    ...
    from .loader import DatasetAttr

    MediaType = Union[PointCloudInput]
                ...
                for i in range(len(medias)):
                    for j in range(len(medias[i])):
                        media_path = os.path.join(
                            self.data_args.media_dir, medias[i][j]
                        )
                        if os.path.isfile(media_path):
...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.dataset_attr.response and isinstance(
        ...
        output = {
            ...
            "_point_clouds": (
                self._find_medias(example[self.dataset_attr.point_clouds])
                if self.dataset_attr.point_clouds
                else None
            ),
        }
        return output
...
@dataclass
class PhysllmDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        if "basic_info" not in example:
            basic_info = {
                "pass_check": False,
                "material": "leather/Faux_Leather_PU",
                "density": 1.1,
                "young": 0.06,
                "hardness": 8,
                "poisson": 0.49,
            ...
            basic_info = example[self.dataset_attr.basic_info]
        kinematic_info = example[self.dataset_attr.kinematic_info]
        part_info = example[self.dataset_attr.part_info]
        object_info = example[self.dataset_attr.object_info]
        ...
        task_name = self.dataset_attr.task_name
        use_image = self.dataset_attr.use_image
        task_handle = get_task_handler(task_name)
        aligned_messages = [{
            "role": Role.USER.value,
            "content": task_handle.build_prompt(),
        }, {
            "role": Role.ASSISTANT.value,
            "content": task_handle.format_label(features),
        }]

        prompt = aligned_messages[:-1]
        response = aligned_messages[-1:]
        system = task_handle.build_system_prompt()
        mm_input = task_handle.build_mm_features(features, media_dir=self.data_args.media_dir, load_from="file")
        output = {
            "_prompt": prompt,
            "_response": response,
            "_system": system,
...
DATASET_CONVERTERS = {
    ...
    "physmeshllm": PhysllmDatasetConverter,
}
