...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __post_init__(self):
        if self.template is None:
            raise ValueError("Template is required for MultiModalDataCollatorForSeq2Seq.")

    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        batch_part_pcds: List[Any] = []
        batch_object_pcds: List[Any] = []
        batch_images: List[Any] = []
        batch_motions: List[dict] = []
        batch_tasks: List[str] = []
        batch_use_image: List[bool] = []
        batch_input_prompts: List[List[dict]] = []

        for feature in features:
            prompts = feature.pop("_prompt")
            responses = feature.pop("_response")
            batch_input_prompts.append(prompts + responses)
            batch_tasks.append(feature.pop("_task_name", None) or "")
            batch_use_image.append(feature.pop("_use_image"))
            # multimodal info
            batch_images.append(feature.pop("_images", None) or [])
            batch_part_pcds.append(feature.pop("_part_point_clouds", None) or [])
            batch_object_pcds.append(feature.pop("_object_point_clouds", None) or [])

            # kinematic info
            batch_motions.append(feature.pop("_motions", None) or {})

        mm_inputs = self.template.mm_plugin.get_mm_inputs(
            part_point_clouds=batch_part_pcds,
            object_point_clouds=batch_object_pcds,
            images=batch_images,
            batch_prompts=batch_input_prompts,
            motions=batch_motions,
            task_name=batch_tasks[0],
            use_image=batch_use_image[0],
        )
        batched_messages = mm_inputs.pop("messages")

        # Encode messages into token ids/labels
        for mi, messages in enumerate(batched_messages):
            ...
            # print("===== decoded from input_ids =====")
            # print(self.tokenizer.decode(input_ids, skip_special_tokens=False))
            # print(self.tokenizer.decode([151652, 151653], skip_special_tokens=False))
            # a = 1
