...
class ModelArguments(
    ...
    task_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "Global task identifier for the model (e.g. 'physics', 'motion', 'group', 'object_level')."
        },
    )
    use_motion_head: bool = field(
        default=False,
        metadata={"help": "Whether use motion head to prediction."},
    )
    use_projector_fusion: bool = field(
        default=False,
        metadata={"help": "Whether use projector for shallow features and llm features."},
    )
    motion_pooling: Optional[str] = field(
        default="mean",
        metadata={
            "help": "Pooling strategy for turning token-level hidden states from llm into a single vector)."
        },
    )

    image_backbone: Literal["none", "clip"] = field(
        default="none",
        metadata={
            "help": "Image backbone to use for the image modality. Choices: ['none', 'clip']."
        },
    )

    point_backbone: Literal["none", "sonata"] = field(
        default="none",
        metadata={
            "help": "point backbone to use for the point cloud modality. Choices: ['none', 'sonata']."
        },
    )
