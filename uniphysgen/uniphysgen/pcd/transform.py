...
from uniphysgen.pcd.registry import Registry
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        # If a shared min_coord is provided (e.g. from the object point cloud),
        # use it so that part and object grid_coords stay in the same reference frame.
        # This is critical for Fourier positional encoding consistency.
        if "shared_grid_min_coord" in data_dict:
            min_coord = np.asarray(data_dict.pop("shared_grid_min_coord"), dtype=int)
        else:
